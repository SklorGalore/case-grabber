...
""")
    ...
    assert window.branch_circuit.record.section == "BRANCH"
    assert window.diagram_stack.currentWidget() is window.branch_circuit
    assert [window.tabs.tabText(i) for i in range(window.tabs.count())] == ["Fields", "Diagram", "Source", "Diagnostics"]
    transformer = Record(
        "TRANSFORMER", 1, [], [["1", "2", "0", "1", "Dyn1"]], [["I", "J", "K", "CKT", "VECGRP"]]
    )
    window._show_record(transformer)
    assert window.diagram.record is transformer
    assert window.diagram_stack.currentWidget() is window.diagram
    window.go_to_bus("2")
    assert window.section == "BUS"
